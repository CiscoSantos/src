...
void imprimi_campo()  // funcao que vai imprimir na tela o campo, tambem ‚ nela que sera feita atribuicao de valores para a matriz do campo. Essa atribuicao e importante porque delimitara os limites do campo;
{
	...
	gotoxy(57,3);
	cprintf("SCORE: %d",score*100);
	for(i=0;i<25;i++) // laco que fara variar as linhas da o campo
	{
		gotoxy(5,i+2); // funcao usada para configura o local na tela onde serao impressos os elementos da matriz;
		for(j=0;j<50;j++) // laco que fara variar as colunas;
		{
			if(campo[i][j]==0)
			{
				printf(" ");
     ...
     switch(opc_lab)
     {
          ...
          case 2:
               for(i=0; i<24; i++)
               {
					for(j=0; j<50; j++)
                    {
                         if(i==0 || i==23 || j==0 || j==49 || ((i>5 && i<20)&& j==24))
                         {
							campo[i][j]=1;
						 }
                         else
          ...
          case 3:
               for(i=0; i<24; i++)
               {
					for(j=0; j<50; j++)
                    {
                         if(i==0 || i==23 || j==0 || j==49 || ((i==8 || i==16) && (j>9 && j<41))) 
						 {
							  campo[i][j]=1;
						 }
                         else
          ...
          case 4:
               for(i=0; i<24; i++)
               {
					for(j=0; j<50; j++)
                    {
                         if(i==0 || i==23 || j==0 || j==49 || ((i>5 && i<19)&& j==24) || ((j>14 && j<34) && i==12))
						 {	
							campo[i][j]=1;
						 }
                         else
...
void imprimi_fruta()
{
	if(comeu==1)
	{
		do
		{
			...
			{
				fruta[0]=sorteia(50);
			}while(fruta[0]==0 || fruta[0]==49);
			do
			{
				fruta[1]=sorteia(25);
			}while(fruta[0]==0 || fruta[0]==24);
		}while(campo[fruta[1]][fruta[0]]==1 || campo[fruta[1]][fruta[0]]==2 );
		    ...
		    case 13://Se usuário digita Enter(código 13 na tabela ASCCI) chama a função correspondente a opção selecionada pelo usuário
			 switch(posicao)
			 {
			      ...
			      case 12:
				   ...
				   break;
			      case 13:
				   aux=aux+1;//Se o usuário não quiser jogar a opção sair, faz com que o jogo não seja executado.
				}
			 break;
	       }
	  }
