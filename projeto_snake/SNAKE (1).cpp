...
int fruta[2];
int atual=0; //atual sera a variavel que quardara o indice da matriz cobra, sempre apontando pra qual gomo do corpo deve ser habilitado
struct lista * rabo;
struct lista * cabeca;
...
void imprimi_cobra() //funcao responsavel por imprimir a cobra na tela;
{
	...
	else  //condicional que ira imprimir a cobra depois que esta for inicializada;
	{
		for(i=0;i<100;i++)// este laca e usado para imprimir os muitos pedacos da cobra
		{
			...
			{	
//				campo[cobra[i].coord_xis][cobra[i].coord_ypsilon]=2; teste pra fazer a cobra morrer quando bate em si mesma
			...
			else break; // condicionalque ira quebrar o la‡o quando os gomos acabarem;
		}
	}
...
//  				                if(campo[cabeca->coord_xis][cabeca->coord_ypsilon]==1 || campo[cabeca->coord_xis][cabeca->coord_ypsilon]==2 ) game_over(); quando a cobra for grande o suficiente pra se matar ao relar nela mesma eu habilito este condicional
